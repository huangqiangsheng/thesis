...
目前，用于计算量子阱吸收谱的商业软件只有一款昂贵的Harold EAM\cite{HarolEAM}。在此，我们采用文献[\citenum{chuang1995physics, mares1993modeling, roy1993modified, chuang1991exciton}]的方法，自己编程计算外界电场下的量子阱的吸收谱。计算吸收谱需要采用四个步骤，如图\ref{fig_ch2_flow_chart}(a)所示。通过第一步和第二布求解电子，空穴在外界下电场下的薛定谔方程\ref{Equ:Schro}得到本征能量$E$和波函数$\psi(z)$，如图\ref{fig_ch2_flow_chart}(b)所示。通过第三步和第四步求解出包含激子效应量子阱的吸收谱，如图\ref{fig_ch2_flow_chart}(c)所示。
\begin{figure}[htb]
	\centering
...
我们用于硅基混合集成III-V电吸收光调制器的多量子阱中，单个量子阱的势阱(Well)和势垒(Barrier)的组分分别是：In\SB{0.65}Al\SB{0.09}Ga\SB{0.26}As和In\SB{0.42}Al\SB{0.17}Ga\SB{0.39}As。从文献[\citenum{chuang1995physics,li2000material}]，根据它们各个元素的组分，再依据\ref{Equ:Econ}，\ref{Equ:Ehh}、\ref{Equ:Elh}、\ref{Equ:eff_mass_z}、\ref{Equ:eff_mass_xy}式可以获得如表\ref{QWmaterial}中的量子阱材料数据。其中势阱受到了0.82\%的压缩应变，势垒受到了0.73\%的拉伸应变。
{
	\begin{table}[htb]
		\zihao{5}
